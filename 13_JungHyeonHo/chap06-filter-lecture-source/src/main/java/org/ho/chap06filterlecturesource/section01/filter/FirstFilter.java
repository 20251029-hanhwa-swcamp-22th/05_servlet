package org.ho.chap06filterlecturesource.section01.filter;
 ...
 *  */
// /first/{아무거나} -> /first/filter가 호출되면 먼저 filter로 이동함
@WebFilter("/first/*")
public class FirstFilter implements Filter {

  /* 필터가 처음 만들어 졌을 때 수행
   * - 한 번 만들어진 필터는 서버가 꺼질 때 까지 계속 유지됨(재사용성)
   * - 서버가 실행될 때 모든 필터가 생성
   * */
  @Override
  ...
  /* 실제 필터링 코드를 작성하고
  * 다음 필터(Filter Chain) 또는 Servlet으로 연결해주는 메서드
  *  */
  @Override
