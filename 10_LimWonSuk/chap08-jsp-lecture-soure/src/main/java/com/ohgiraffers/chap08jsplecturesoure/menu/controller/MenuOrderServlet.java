package com.ohgiraffers.chap08jsplecturesoure.menu.controller;
...
public class MenuOrderServlet extends HttpServlet {
    ...
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) // HttpServletResponse = 응답 객체 String 포함
            throws ServletException, IOException {
        /* 1. 요청에 대한 처리(전달 받은 값 추출, 검증) */
        req.setCharacterEncoding("UTF-8");

        // amount값 parseInt해서 가져오는거
        ...
        *       -> 값(상테) 유지 범위 : 현재 Servlet + 위임 받는 JSP
        * */
        ...
        /* 4. 응답 페이지 생성 후 응답
        * - PrintWriter를 이용하여
        *       직접 문자열로 HTML코드를 작성, 출력하는 것은 힘드므로
        * - HTML 코드 작성에 용이한 JSP로 요청을 위임하여
        *       대신 응답 페이지를 만들어 출력하게 함
        * */

        // 요청 발송자 객체 생성 (요청 주소 또는 jsp파일 경로)
        RequestDispatcher rd
                = req.getRequestDispatcher("/jsp/response.jsp");

        rd.forward(req, resp); // 요청 위임 respons.jsp에게
    }
}
